...
/**
 * Hook to get custom avatars url (Buddypress specific)
 */
function oa_social_login_bp_custom_fetch_avatar_url($text, $args)
{
    //The social login settings
    static $oa_social_login_avatars = null;
    if (is_null($oa_social_login_avatars))
        ...
        if (is_array($args))
        {
            ...
            if (!empty($args['object']) and strtolower($args['object']) == 'user')
            {
                ...
                if (!empty($args['item_id']) and is_numeric($args['item_id']))
                {
                    ...
                    if (($user_data = get_userdata($args['item_id'])) !== false)
                    {
                        // prevent hook fired loop
                        remove_filter('bp_core_fetch_avatar_url', 'oa_social_login_bp_custom_fetch_avatar_url', 10);
                        ...
                        add_filter('bp_core_fetch_avatar_url', 'oa_social_login_bp_custom_fetch_avatar_url', 10, 2);
                    }
                }
            }
        }
    ...
    add_filter('clean_url', 'oa_social_login_bp_custom_fetch_avatar_url_esc', 10, 3);

    return $text;
}
add_filter('bp_core_fetch_avatar_url', 'oa_social_login_bp_custom_fetch_avatar_url', 10, 2);

function oa_social_login_bp_custom_fetch_avatar_url_esc($good_protocol_url, $original_url, $_context)
{
    return $original_url;
}
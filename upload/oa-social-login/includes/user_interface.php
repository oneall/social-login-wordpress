            ...
            if (!isset ($settings ['plugin_profile_show']) OR !empty ($settings ['plugin_profile_show']))
            {
                ...
                if (!empty ($api_subdomain))
                {
                    ...
                    else
                    {
                        ...
                        $containerid = 'oneall_social_login_providers_' . wp_rand (99999, 9999999);
...
function oa_social_login_render_login_form ($source, $args = array ())
{
    ...
    if (!empty ($api_subdomain))
    {
        ...
        else
        {
            ...
            {
                $output [] = ' <div class="oneall_social_login_label" style="margin-bottom: 3px;"><label>' . $plugin_caption . '</label></div>';
            }
                    ...
                    <div class="oa_social_login_modal_outer">
                        <div class="oa_social_login_modal_inner">
                            <div class="oa_social_login_modal_title">
                                <?php
                                /* translators: %s: provider name */
                                $message = sprintf (__ ('You have successfully connected with %s!', 'oa-social-login'), '<strong>' . $oa_social_login_identity_provider . '</strong>');
                                /* phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped */
                                echo $message;
                                ?>
                            </div>
                            ...
                            {
                                ?>
                                <div class="oa_social_login_modal_notice">
                                    <?php
                                    /* translators: %s: provider name */
                                    $message = str_replace ('%s', $oa_social_login_identity_provider, $caption);
                                    /* phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped */
                                    echo $message;
                                    ?>
                                </div>
                            ...
                            <div class="oa_social_login_modal_body">
                                <div class="oa_social_login_modal_subtitle">
                                    <?php _esc_attr_e ('Please enter your email address', 'oa-social-login'); ?>:
                                </div>
                                <form method="post" action="">
                                    <fieldset>
                                        <div class="oa_social_login_input">
                                            <input type="text" name="oa_social_login_email" class="oa_social_login_confirm_text" value="<?php /* phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped */ echo (!empty ($_POST ['oa_social_login_email']) ? oa_social_login_esc_attr ($_POST ['oa_social_login_email']) : ''); ?>" />
                                            <input type="hidden" name="oa_social_login_action" value="confirm_email" size="30" />
                                        </div>
                                        <div class="oa_social_login_modal_error">
                                            <?php /* phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped */ echo $message; ?>
                                        </div>
                                        <div class="oa_social_login_buttons">
                                            <input class="oa_social_login_button" id="oa_social_login_button_confirm" name="oa_social_login_confirm_btn" type="submit" value="<?php _esc_attr_e ('Confirm', 'oa-social-login'); ?>" />
                                            <a href="<?php /* phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped */ echo esc_url ($logout_url); ?>" class="oa_social_login_button" id="oa_social_login_button_cancel"><?php _esc_attr_e ('Cancel', 'oa-social-login'); ?></a>
                                        </div>
                                    </fieldset>
                                </form>
                            </div>
